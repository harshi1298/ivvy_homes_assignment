...
Full-stack property portal frontend and automated data-auditing pipeline developed for the Ivy Homes Software Engineering Internship assignment (Bengaluru, September 2026).
...
```
IvyProject/
├── .env.example              # Template for API credentials and candidate info
├── .env                      # Local environment secrets (ignored by git)
├── scripts/
│   ├── harvest.js            # Automated crawler for full city property dataset
│   └── audit.js              # Algorithmic solver for 10 Questions & API Discrepancy Hunter
├── src/
...
Ensure your `.env` has your registered details:
```env
...
CANDIDATE_EMAIL=your_email@mnnit.ac.in
CANDIDATE_REPO_URL=https://github.com/your-username/ivy-homes-assignment
CANDIDATE_DEMO_URL=https://your-deployment.vercel.app
```
...
### 3. Run Data Harvester & Solver Pipeline
```bash
# 1. Download full city dataset (Listings, Rentals, Projects, Health, Analytics)
npm run harvest

# 2. Compute the 10 Questions and compile submission.json
npm run audit
...
1. **Interactive Geospatial Map View:** Mapbox/Leaflet integration with cluster markers and price heatmaps across Chennai localities (OMR, Adyar, Velachery, T Nagar).
2. **Broker Anomaly Correction Portal:** A workflow enabling verified brokers to submit corrections for flagged corrupt records (e.g. correcting swapped floor numbers or inverted area dimensions).
3. **Automated Price Valuation Model:** A lightweight valuation model trained on verified locality carpet rates to highlight underpriced bargains and alert buyers to overpriced units.
4. **Offline-First IndexedDB Synchronization:** Full client-side offline storage using IndexedDB so the portal functions seamlessly under low-connectivity mobile environments.
